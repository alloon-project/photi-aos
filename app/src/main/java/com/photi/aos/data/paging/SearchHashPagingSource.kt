...
class SearchHashPagingSource(private val hash: String, private val searchViewModel: SearchViewModel, private val challengeRepository: ChallengeRepository) : PagingSource<Int, ChallengeData>() {

    override suspend fun load(params: LoadParams<Int>): LoadResult<Int, ChallengeData> {
        val page = params.key ?: 0 // 첫 페이지는 0부터 시작

        return try {
            Log.d("SearchHashPagingSource", "Search History Loading page: $page, pageSize: ${params.loadSize}")

            val response = challengeRepository.getSearchHashtag(hash, page, params.loadSize)
            val data = response.body()?.data

            if (data == null) {
                Log.e("SearchHashPagingSource", "API 응답 데이터가 null입니다.")
            ...
            Log.d("SearchHashPagingSource", "Loaded ${content.size} items")
            ...
            Log.e("SearchHashPagingSource", "Error: ${exception.message}")
